...
class EditarVehiculoYear extends Component {
    ...
    get_data() {
        axios.get( web.servidor + '/vehiculo_year/edit/' + this.props.match.params.id).then(
            (response) => {
    ...
    onChangeDescripcion(event) {
        if (!isNaN(event.target.value)) {
            ...
            message.error('SOLO SE PERMITE NUMERO');    
        }
    }
    onBack() {
    ...
    onSubmit() {
        ...
        axios(
            {
                method: 'post',
                url: web.servidor + '/vehiculo_year/update',
                data: formdata,
                headers: {
...
EditarVehiculoYear.propTypes = {
    buttoncolor: PropTypes.string,
}

EditarVehiculoYear.defaultProps = {
    buttoncolor: '',
}

export default withRouter(EditarVehiculoYear);
