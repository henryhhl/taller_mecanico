...
class EditarVehiculoTipo extends Component {
    ...
    get_data() {
        axios.get( web.servidor + '/vehiculo_tipo/edit/' + this.props.match.params.id).then(
            (response) => {
    ...
    onSubmit() {
        ...
        axios(
            {
                method: 'post',
                url: web.servidor + '/vehiculo_tipo/update',
                data: formdata,
                headers: {
                ...
                if (response.status == 200) {
                    if (response.data.response == 1) {
                        notification.success({
                            message: 'SUCCESS',
                            description: 'VEHICULO TIPO ACTUALIZADO EXITOSAMENTE',
                        });
                        this.props.history.goBack();
                    }
                    if (response.data.response == -1) {
                        notification.error({
                            message: 'Advertencia',
                            description: 'NO SE PERMITE TIPO REPETIDO',
                        });
                        this.setState({ errordescripcion: 'error', });
                    }
                }
...
EditarVehiculoTipo.propTypes = {
    buttoncolor: PropTypes.string,
}

EditarVehiculoTipo.defaultProps = {
    buttoncolor: '',
}

export default withRouter(EditarVehiculoTipo);
