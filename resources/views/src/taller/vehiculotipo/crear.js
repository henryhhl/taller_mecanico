...
class CreateVehiculoTipo extends Component {
    ...
    get_data() {
        axios.get( web.servidor + '/vehiculo_tipo/create').then(
            (response) => {
                ...
                if (response.status == 200) {
                    if (response.data.response == 1) {
                        notification.success({
                            message: 'SUCCESS',
                            description: 'VEHICULO TIPO CREADO EXITOSAMENTE',
                        });
                        this.props.history.goBack();
                    }
                    if (response.data.response == -1) {
...
CreateVehiculoTipo.propTypes = {
    buttoncolor: PropTypes.string,
}

CreateVehiculoTipo.defaultProps = {
    buttoncolor: '',
}

export default withRouter(CreateVehiculoTipo);
