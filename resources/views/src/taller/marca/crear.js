...
class CreateVehiculoMarca extends Component {
    ...
    get_data() {
        axios.get( web.servidor + '/vehiculo_marca/create').then(
            (response) => {
                ...
                if (response.status == 200) {
                    if (response.data.response == 1) {
                        notification.success({
                            message: 'SUCCESS',
                            description: 'MARCA CREADO EXITOSAMENTE',
                        });
                        this.props.history.goBack();
                    }
                    if (response.data.response == -1) {
...
CreateVehiculoMarca.propTypes = {
    buttoncolor: PropTypes.string,
}

CreateVehiculoMarca.defaultProps = {
    buttoncolor: '',
}

export default withRouter(CreateVehiculoMarca);
